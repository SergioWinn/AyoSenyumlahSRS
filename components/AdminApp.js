...
export default function AdminApp() {
  ...
  return <main className="admin-shell"><header className="admin-header"><div><a className="wordmark" href="/">Ayo Senyumlah</a><h1>Sinkronisasi data</h1><p>Data lama tetap tersimpan bila API gagal atau sudah hilang.</p></div><button className="secondary-button" onClick={() => sync()}>Sinkronkan semua</button></header>{message && <p className="notice" role="status">{message}</p>}<div className="source-list">{status.sources?.map((source) => <article className="source-row" key={source.id}><div><strong>{source.group_name} · {source.ticket_type}</strong><small>{source.last_synced_at ? `Terakhir ${new Date(source.last_synced_at).toLocaleString("id-ID")}` : "Belum pernah disinkronkan"}</small>{source.last_sync_error && <small className="error-text">{source.last_sync_error}</small>}</div><button className="secondary-button" onClick={() => sync(source.id)}>Sinkronkan</button></article>)}</div></main>;
}
