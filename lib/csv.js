export const CSV_COLUMNS = ["Member", "Sesi", "Jalur", "Tipe Tiket"];

export function buildScheduleCsvTemplate(slots) {
  const lines = [CSV_COLUMNS, ...slots.slice(0, 2).map((slot) => [slot.member_name, slot.session_label, slot.lane_label, slot.ticket_type])];
...
export function parseScheduleCsv(text, slots) {
  ...
  for (const line of parsed.slice(1)) {
    const [member, session, lane, ticket] = indexes.map((index) => norm(line[index]));
    const found = slots.find((slot) => norm(slot.member_name) === member
      && norm(slot.session_label) === session
      && (!lane || norm(slot.lane_label) === lane)
