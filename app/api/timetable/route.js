...
export async function GET() {
  ...
  const { data: event, error: eventError } = await supabase
    .from("events").select("id,name,event_date,venue").eq("is_active", true)
  ...
  const { data: slots, error } = await supabase
    .from("session_slots")
    .select("id,session_label,lane_label,member_name,group_name,ticket_type,sort_order,schedules(id,participant_name)")
    .eq("event_id", event.id).eq("is_active", true)
    .order("sort_order").order("member_name");
